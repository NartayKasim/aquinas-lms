...
import ShowModulePacked from './ShowModulePacked';
...
import ShowModuleUnpacked from './ShowModuleUnpacked';
    ...
    return (
        <div className="unit-page">
            <div className="unit-module" >
                <ShowBlock id="reduce-margin" type={"UNIT"} title={title} description={description} courseParam={props.match.params.courseID} content={content} key={uuidv4()} />
            </div >
            <div className="unit-module--units">
                ...
                {content.length === 0 ? null : <span className="unit-module--units__hint">(click on a module to see its contents)</span>}

                {
                    content.length === 0
                        ?
                        <div className="no-modules-message">
                            <span>It looks like there are no modules in this unit.</span>
                        </div>
                        :
                        content.map((mdl, mIdx) => {
                            if (mIdx === 0 && mIdx + 1 !== content.length) {
                                return <ShowModulePacked
                                    key={uuidv4()}
                                    title={mdl.title}
...
export default requireAuth(ShowUnit);
